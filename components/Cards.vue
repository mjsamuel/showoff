<template>
  <div
    class="relative z-0 m-auto flex h-full w-full justify-center sm:mb-24 sm:mt-10">
    <TransitionGroup
      ...
      <div
        ...
        <div
          class="sm:w-160 flex h-full w-full flex-col items-center rounded-t-xl bg-gray-100 shadow-xl sm:flex-row sm:bg-transparent sm:shadow-none"
          :class="{ 'sm:justify-center': !card.modifier }">
          <!-- category -->
          <div
            class="h-fit w-96 p-5 sm:static sm:h-fit sm:min-h-96 sm:w-80 sm:rounded-xl sm:bg-gray-100 sm:shadow-xl"
            :class="`sm:${card.category.rotation < 0 ? '-' : ''}rotate-${Math.abs(card.category.rotation)}`">
          ...
          <Transition
            ...
            <div
              v-if="
                !!card.modifier && (index < cards.length - 1 || showModifier)
              "
              class="h-full w-96 rounded-t-xl border-black bg-gray-700 p-5 shadow-xl sm:h-fit sm:min-h-96 sm:w-80 sm:rounded-xl"
              :class="`sm:${card.modifier.rotation < 0 ? '-' : ''}rotate-${Math.abs(card.modifier.rotation)}`">
